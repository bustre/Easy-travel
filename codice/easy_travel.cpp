...
operator<<(std::ostream& os, const QueryResult& q_result)
{
  auto generate_space = [] (std::string already_printed, int n) -> std::string {
    std::string result {};

    n = n - already_printed.length();
  ...
  // Lunghezza massima per  ogni colonna colonna
  ...
  for (int col = 0; col < q_result.n_fields; ++col) {
    std::string header {PQfname(q_result.result, col)};
    os << "| "
       << header
       << generate_space(header,col_widths[col])
       << " ";
  }
  os << "|" << "\n";
  ...
  for (int i = 0; i < q_result.n_tuple; ++i) {
    for (int j = 0; j < q_result.n_fields; ++j) {
      std::string str_tuple {PQgetvalue(q_result.result, i, j)};
      os << "| " 
         << str_tuple
         << generate_space(str_tuple,col_widths[j])
         << " ";
    }
    os << "|" << "\n";
  }
...
//////////////////// Opzioni del menu & menu applicazione ////////////////////
...
run_query_1(Database& db)
{
  ...
  std::cout <<
    R"(
Trovare tutti i clienti che hanno effettuato almeno una recensione con un voto superiore o uguale a x stelle.
Si riporti: il nome, cognome e l'email del cliente. Inoltre specificarne il nome dell'alloggio per cui è stata
scritta la recensione, il giudizio dato e la motivazione se disponibile, nel caso tale informazione non fosse 
disponibile sostituire NULL con la stringa '* Nessuna motivazione fornita'.
)";

  int stars = console_input<int>(
    "\nInserire quante stelle devono avere le recesioni della "
    "query (0 o un numero negativo per non filtrarle): ");

  std::cout << "\n\n"; // Padding
  try {
    std::cout << db.exec_query("Query 1", { std::to_string(stars) });
...
che sono stati prenotati con quella compagnia aerea, la media prezzi dei voli offerti, il prezzo massimo devi voli offerti
...
almeno x voli. In fine si riordini in modo decrescente per numero voli.
  ...
  int n_flights = console_input<int>("\nInserire il valore di x: ");

  std::cout << "\n\n"; // Padding
  try {
    std::cout << db.exec_query("Query 2", { std::to_string(n_flights) });
  } catch (const std::runtime_error& err) {
    std::cout
      << "ERRORE: c'e' stato un errore nel tenativo di eseguire la query :(\n"
      << err.what();
...
Si includano inoltre anche i casi in cui non si e' prenotato il trasporto attraverso il servizio, e si riporti in quei
casi il valore 0, sia per il totale trasporto che per il numero di voli.
...
tutti i pacchetti che hanno il prezzo in un certo intevervallo. Ordinarli per data di partenza in ordine crescente.
...
il codice del volo di arrivo, la data/ora di arrivo, il numero di scali, il prezzo contando tutti i voli
...
colonne sono state rinominate in modo differente rispetto alla relazione per motivi di spazio.
  ...
  try {
    std::cout << db.exec_query(
      "SELECT A.CODICE AS AEROPORTO_CODICE, C.NOME AS UBICAZIONE, C.STATO FROM "
      "AEROPORTO AS A JOIN CITTA AS C ON C.ID = A.ID_CITTA;");
  } catch (const std::runtime_error& err) {
    std::cout
      << "ERRORE: c'e' stato un errore nel tenativo di eseguire la query :(\n"
      << "\t dettagli: " << err.what();
  ...
  std::string departure = console_input<std::string>(
    "Inserire il codice dell'aerporto di partenza: ");
  std::string destination =
    console_input<std::string>("Inserire il codice dell'aerporto di arrivo: ");

  try {
    std::cout << db.exec_query("Query 6", { departure, destination });
  } catch (const std::runtime_error& err) {
    std::cout
      << "ERRORE: c'e' stato un errore nel tenativo di eseguire la query :(\n"
      << "\t dettagli: " << err.what();
...
main_menu(Database& db)
{
  ...
  do {
    ...
    switch (select) {
      ...
      default:
        std::cout << "ERORR: l'opzione selezionata e' inesistente.\n";
        break;
    }
...
prepared_query(Database& db)
{
  ...
  // Rinominiamo alcune clonne rispetto alla relazione per fare in modo che la tabella ci stia nel terminale
  db.prepared_query("Query 6",
        ...
        SELECT
            ...
            DIRETTO.TIMESTAMP_PARTENZA AS D_PART,
            DIRETTO.TIMESTAMP_ARRIVO AS D_ARR,
            DIRETTO.TOTALE_PREZZO AS D_PREZ,
            SCALI.PRIMO_VOLO AS S_PART_COD,
            SCALI.VOLO_ATTUALE AS S_ARR_COD,
            V.TIMESTAMP_PARTENZA AS S_PART,
            SCALI.TIMESTAMP_ARRIVO AS S_ARR,
            SCALI.NUMERO_SCALI AS N_SCALI,
            SCALI.TOTALE_PREZZO AS S_PREZ,
...
init_app(void)
{
  std::string user = console_input<std::string>("Inserire il nome utente: ");
  std::string password = console_input<std::string>("Inserire la password: ");

  std::cout << "Connessione al database...\n";
  Database db = Database("127.0.0.1", 5432, user, "Easy Travel", password);
  ...
  std::cout << "\nERROR: c'e' stato un errore nel tentativo di connettermi al "
               "database:\n";
  std::cout << db.get_error_msg() << "\n\n";
...
main(void)
{
  ...
  try {
    prepared_query(easy_travel);
  } catch (std::runtime_error& err) {
    std::cout << "\n\n\nERROR: c'e' stato un problema nel tentativo di inviare "
                 "le query al server. Il programma verra' terminato.\n"
              << err.what() << "\n";
    easy_travel.close();
    exit(1);
  }
